...
zround (double r)
{
  ...
  /* R can be outside the range of an int if glue is stretching or
     shrinking a lot.  We can't do any better than returning the largest
     or smallest int possible in that case.  It doesn't seem to make
	...
	\eject
     ...
     TeX's largest or smallest int (32 bits), not the machine's.  So
